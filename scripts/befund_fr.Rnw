...
\vspace{-0.3cm}
\begin{tabbing}
\textbf{Nom du patient:  } \=\Sexpr{paste(svalue(p.name), svalue(p.vorname), sep=", ")} \hspace{7.5cm} \= \textbf{Date of birth:} \=\Sexpr{svalue(p.geb)} \\ % geburtsdatum des Patienten
...
\\
\textbf{Date du diagnostic:} \>\Sexpr{svalue(p.datediag)}
\\
...
% genes
\begin{large}\textbf{Gènes et de cibles exprimés}\end{large} \\
\line(1,0){170}
