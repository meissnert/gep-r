...
% Heading
\Betreff	{Analyse du transcriptome des cellules plasmocytaires tumorales par puces Affymétrix (GEP-R)}
...
\vspace{-0.3cm}
\begin{tabbing}
\textbf{Nom du patient:  } \=\Sexpr{paste(svalue(p.name), svalue(p.vorname), sep=", ")} \hspace{3.5cm} \= \textbf{Date de naissance:} \=\Sexpr{svalue(p.geb)} \\ % geburtsdatum des Patienten
\textbf{Adresse:} \>\Sexpr{paste(svalue(p.strasse), svalue(p.ort), svalue(p.plz), svalue(p.country), sep=", ")}
\end{tabbing}
\vspace{-0.6cm}
\line(1,0){170}
\begin{tabbing}
\textbf{Pathologie:} \hspace{1cm} \=\Sexpr{svalue(p.diag)} \hspace{1cm} \= \textbf{Stade Salmon Durie:} \=\Sexpr{svalue(p.stage)}%hier klini. Diagnose eintragen
\\
\textbf{Date du diagnostic:} \hspace{3cm} \=\Sexpr{svalue(p.datediag)}
\\
\textbf{Données cliniques:}
\\
\=\textbf{Type de chaînes lourdes:} \Sexpr{svalue(p.igtype)} % IG-Typ eintragen
\\
\>\textbf{Type de Chaînes légères:} \Sexpr{svalue(p.lk)} % leichtkette eintragen
\end{tabbing}

% information about genexpression analysis
\begin{tabular}{ | l l | } \hline
Date de la ponction: & \Sexpr{svalue(probe.date)} \tabularnewline 
Volume de l'échantillon: & \Sexpr{svalue(probe.volume)} ml\tabularnewline 
Technique de purification CD-138 (autoMACS): & \Sexpr{svalue(probe.protokoll)}  \tabularnewline
Pureté en CD-138 par cytometrie: & \Sexpr{svalue(probe.purity.facs)} \% \tabularnewline
Pureté en CD-138 par FISH: & \Sexpr{svalue(probe.purity.fish)} \% \tabularnewline
Quantité d'ARN utilisé: & \Sexpr{svalue(probe.rna)} ng \tabularnewline
Type de puce: & \Sexpr{svalue(probe.array)} \tabularnewline
Nombre de probesets analysés: & \Sexpr{process.res$genes} \tabularnewline
...
% report
\begin{large}\textbf{Rapport}\end{large} \\
\line(1,0){170}
\begin{enumerate}
\item \textbf{Contrôle Qualité:} 
\Sexpr{svalue(befund.qualitycontrol)}

\item \textbf{Contrôle d'identité:} 
\Sexpr{svalue(befund.identitycontrol)}

\item \textbf{Gènes fréquemment surexprimés ou exprimés de façon aberrante dans le myélome multiple} \\
\Sexpr{svalue(befund.genes)}

\item \textbf{Signatures moléculaires :} \\
\Sexpr{svalue(befund.classification)}

\item \textbf{Classification par score de risque:} \\
\Sexpr{svalue(befund.risk)}

\item \textbf{Cible(s) pour l'immunothérapie:} \\
\Sexpr{svalue(befund.itherapy)}

\item \textbf{Cible(s) pour un traitement individualisé:} \\
\Sexpr{svalue(befund.grtherapy)}

\item \textbf{Réarrangements chromosomiques:} \\
...
% Conclusion
\begin{large}\textbf{Conclusions}\end{large} \\
\vspace{-0.25cm}
\line(1,0){170}
...
\begin{tabbing}
Pr. B. KLEIN \hspace{3.2cm} \=Dr. J. Moreaux \\
\end{tabbing}
...
\textbf{\begin{Large}Annexes\end{Large}}

% qualitycontrol
\begin{large}\textbf{Contrôles qualité}\end{large} \\
\vspace{-0.3cm}
\line(1,0){170}
\vspace{-0.1cm}
\begin{center}
...
% identitycontrol
\begin{large}\textbf{Contrôle d'identité}\end{large} \\
\line(1,0){170}

\vspace{0.5cm}

\begin{tabular}{ | l | c | c | } \hline
 & Prediction & Possibilités \tabularnewline \hline \hline
Sexe & \Sexpr{prediction.res[[1]]} & [m, f] \tabularnewline \hline 
Chaînes lourdes & \Sexpr{prediction.res[[2]]} & [A, G, D] \tabularnewline \hline 
Chaînes légères & \Sexpr{prediction.res[[3]]} & [lambda, kappa]  \tabularnewline \hline 
\end{tabular}

\vspace{1cm}

% risk stratification
\begin{large}\textbf{Classification par score de risque}\end{large} \\
\line(1,0){170}

\vspace{0.5cm}

\begin{tabular}{ | l | c | c |} \hline
 & Classification & Possibilités \tabularnewline \hline \hline
...
Gene expression based proliferation index (Hose et al., 2008) & \Sexpr{risk.res[[7]]} & [high, medium, low] \tabularnewline \hline 
...
\begin{large}\textbf{Classification moléculaire}\end{large} \\
\line(1,0){170}

\vspace{0.5cm}

\begin{tabular}{ | l | c | c |} \hline
 & Groupe & Possibilitiés \tabularnewline \hline \hline
Molecular classification (Zhan et al, 2006) & \Sexpr{risk.res[[2]]} & [PR, LB, MS, HY, CD1, CD2, MF] \tabularnewline \hline
TC classification (Bergsagel, 2005) & \Sexpr{risk.res[[4]]} & [4p16, maf, 6p21, 11q13, d1, d1d2, d2, none] \tabularnewline \hline
...
% genes
\begin{large}\textbf{Gènes et cibles exprimés}\end{large} \\
\line(1,0){170}
...
\multicolumn{8}{ | l | }{\textbf{Gènes fréquemment surexprimés dans le myelome multiple}} \tabularnewline \hline \hline
...
\multicolumn{8}{ | l | }{} \tabularnewline
\multicolumn{8}{ | l | }{\textbf{Cibles pour l'immunothérapie}} \tabularnewline \hline \hline
...
\multicolumn{8}{ | l | }{} \tabularnewline
\multicolumn{8}{ | l | }{\textbf{Cibles pour un traitement à la carte}} \tabularnewline \hline \hline
