...
\hspace{8cm}
\textbf{Date of birth:} \Sexpr{svalue(p.geb)} % geburtsdatum des Patienten
\\
\line(1,0){170}
\begin{tabbing}
...
Samplevolume: & \Sexpr{svalue(probe.volume)} ml\tabularnewline 
CD-138 purification (autoMACS): & \Sexpr{svalue(probe.protokoll)}  \tabularnewline
CD-138 purity (FACS)\footnote[1]{Test}: & \Sexpr{svalue(probe.purity)} \% \tabularnewline
Amount of RNA used: & \Sexpr{svalue(probe.rna)} ng \tabularnewline
Array-Type: & \Sexpr{svalue(probe.array)} \tabularnewline
Amount of analyzed probesets: & \Sexpr{nr.genes} \tabularnewline
Amount of analyzed genes and ESTs: & ca. 39 000 \tabularnewline
RNA amplification protocoll: & \Sexpr{svalue(probe.ampl)} \tabularnewline
...
\item \textbf{Genes over- or aberrantly expressed} \\
\Sexpr{svalue(befund.genes)}

%\item \textbf{Classification of Myeloma} \\
%\Sexpr{svalue(befund.classification)}

\item \textbf{Risk stratification} \\
\Sexpr{svalue(befund.risk)}

\item \textbf{Targets for immun-therapy} \\
...
Head, Multiple Myeloma Research Lab \> Head, Section Multiple Myeloma
\end{tabbing}
...
\line(1,0){170}
\begin{center}
\resizebox{7.5cm}{!}{\includegraphics{temp/artifacts.png}}
\hspace{1cm}
\resizebox{7.5cm}{!}{\includegraphics{temp/qcsummary.png}}
\end{center}
...
\begin{tabular}{ | l | c | c | } \hline
 & Prediction & Range \tabularnewline \hline \hline
Sex & \Sexpr{sex} & [m;f] \tabularnewline \hline 
IgH-type & \Sexpr{type} & [A;G;D] \tabularnewline \hline 
IgL-type & \Sexpr{lightchain} & [lambda;kappa]  \tabularnewline \hline 
\end{tabular}

\vspace{1cm}

% molecular classification
\begin{large}\textbf{(Molecular) Classification}\end{large} \\
\line(1,0){170}

\vspace{0.5cm}

\begin{tabular}{ | l | c | c |} \hline
 & Classification & Range \tabularnewline \hline \hline
Bergsagel TC & \Sexpr{bergsagel} & [4p16;maf;6p21;11q13;d1;d1d2;d2;none] \tabularnewline \hline
Hose EC & \Sexpr{ec} & [1.1; 1.2; 2.1; 2.2] \tabularnewline \hline
Zahn Molecular &  & [...8 Gruppen...] \tabularnewline \hline
\end{tabular}
...
Gene expression based proliferation index (Hose et al., 2008) & \Sexpr{gpi} & [high;medium;low] \tabularnewline \hline 
\end{tabular}
